 ...
 * installed foundational modules), inventory, payments, delivery, and pos
 * (opt-in business modules — a tenant installs them explicitly via POST
 * /api/modules/{inventory,payments,delivery,pos}/install; none are
 * auto-installed by bootstrap-tenant.ts). pos depends on both inventory
 * (Phase 9 retrofit) and payments (Phase 10 retrofit — see
 * modules/pos/src/module.manifest.ts); delivery (Phase 11) depends only
 * on core/tenant/identity — it references orders by an opaque string, not
 * a foreign key, since no sales module exists (see
 * modules/delivery/src/module.manifest.ts). validateGraph() runs once
