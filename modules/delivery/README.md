...
Phase 13 adds `application/order-paid-consumer.ts` — reacts to `pos`'s `OrderPaid` event by idempotently creating a pending Delivery (EVENTS.md §6's worked example). Every paid order currently triggers this unconditionally — there's no real "requires delivery" signal on checkout yet.
