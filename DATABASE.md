...
**Tenant-side migrations (implemented, Phase 4; wired into module installation, Phase 7):** `packages/database` exposes `runTenantMigrations(tenantId, migrationsFolder, migrationsTable)`, which applies a pre-generated SQL migration set (via `drizzle-orm/postgres-js/migrator`) to one tenant's database over a short-lived connection. This is a minimal, module-agnostic primitive — each module owning tenant-DB tables sets it as its `ModuleManifest.applyMigrations` hook (e.g. `modules/identity/src/apply-migrations.ts`), and `modules/core`'s `installModule` calls it as step 3 of MODULE-SYSTEM.md §3's 10-step sequence.

Two real bugs were found and fixed here via live verification (not caught by automated tests, which don't exercise the same runtime paths — CLAUDE.md §56):

- **Phase 8 (Turbopack path resolution):** the migrations folder must be resolved relative to the calling file at runtime. `import.meta.dirname` is `undefined` under Turbopack's Route Handler runtime, and `new URL("../migrations", import.meta.url)` gets statically intercepted by Turbopack as an asset-import directive and fails to resolve a non-JS directory. Fixed by resolving `fileURLToPath(import.meta.url)` as a standalone expression, then joining "../migrations" with plain `path.join` — see `modules/identity/src/apply-migrations.ts`'s doc comment for the full history. Still a dev-mode-only fix: a `next build` production bundle doesn't copy non-imported `*.sql` files into its output, so real production deployment needs proper migration-asset packaging (untouched gap).
- **Phase 9 (shared migration watermark):** `migrationsTable` must be unique per module. Drizzle's migrator tracks "already applied" by comparing each local migration's *generation* timestamp against a single highest-watermark timestamp in the tracking table — not by folder identity. With every module sharing the default `__drizzle_migrations` table, a module whose migration happened to be *generated* earlier than another module's (regardless of install order) got silently skipped as "already applied" the moment it installed after that other module — `installModule` reported success even though the module's tables never got created. Fixed by giving every module its own tracking table (`__drizzle_migrations_<module>`) — see `packages/database/src/tenant/migrate.ts`'s doc comment.
