...
This repository has completed **Phase 15 (SaaS Billing)**: `modules/billing` makes real use of the `plans`/`subscriptions`/`billing` control-plane tables that had sat as unused Phase 2 placeholders until now. A tenant subscribes to a plan (`starter` or `growth`, matching CLAUDE.md §1's Tenant A/B illustration); `modules/core`'s `installModule` now accepts an injected `EntitlementChecker` port (default: allow-all, so every pre-existing caller is unaffected) and `POST /api/modules/[moduleId]/install` wires in billing's real `SubscriptionEntitlementChecker` — the same Ports & Adapters seam `modules/pos` uses for stock reservation and payment capture. Live-verified end to end: a fresh tenant can install a module its plan includes (200) and is denied one it doesn't (402 `MODULE_NOT_ENTITLED`). No real payment gateway (`StubPaymentGateway`, same precedent as `modules/payments`' `SimulatedCardProvider`) and no platform-admin HTTP API (plans are seeded via an idempotent script). See [docs/modules/billing.md](./docs/modules/billing.md), and [CLAUDE.md](./CLAUDE.md) for the full governing specification and [docs/adr/](./docs/adr/) for the architecture decisions made so far.
