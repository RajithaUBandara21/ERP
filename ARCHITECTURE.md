...
14. Reporting             ✓ done — modules/reporting, a read model (SalesDailySummary) populated by a second, independent OrderPaid consumer alongside delivery's (EVENTS.md §6). Depends only on core/tenant/identity, not pos/inventory/payments/delivery — no direct queries against other modules' live tables (CLAUDE.md §10/§52), only event subscription plus its own pre-aggregated table. GET /api/reporting/sales-summary is cursor-paginated (packages/validation's paginationCursorSchema) and permission-gated (REPORTING.SALES.READ). Live-verified: a POS checkout followed by publishing the outbox correctly updates the sales summary, independently of whether delivery is even installed for that tenant.
15. SaaS billing          ✓ done — modules/billing (plans, subscriptions, billing charges — all control-plane data, DOMAIN-MODEL.md §3). Real entitlement enforcement: modules/core's installModule accepts an injected EntitlementChecker port (default: AllowAllEntitlementChecker, so every pre-existing caller is unaffected), and billing's SubscriptionEntitlementChecker is the real implementation, wired in by apps/web's install route — the same Ports & Adapters seam as modules/pos's StockReservationPort. Two default plans (starter, growth) match CLAUDE.md §1's Tenant A/B illustration; starter must include payments alongside pos/inventory since pos hard-depends on it. Fixed a real gap in the Phase 2 placeholder schema along the way: subscriptions had no unique index on tenant_id (nothing had used the table yet) — added via migration before writing the idempotent-per-tenant createSubscription against it. No real payment gateway (StubPaymentGateway, same precedent as payments' SimulatedCardProvider) and no platform-admin auth (plans are seeded via script, not an HTTP API) — see docs/modules/billing.md's Implementation status for the full deferred-scope list.
16. Observability         ← current phase
17. Performance
